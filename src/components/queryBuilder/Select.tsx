...
export type MultiSelectMetricPropertyTag = {
...
export const MultiSelectMetricPropertyTag = (props: MultiSelectMetricPropertyTag) => {
...
export type SingleSelectAdapterResourceKind = {
    fetchedOptions: string[];
    value: string;
    changeFunction: SingleChangeFunction;
    labels: Labels;
...
export type AdapterResourceKind = {
    datasource: DataSource;
    builderOptions: QueryBuilderOptions;
...
export const AdapterResourceKind = (props: AdapterResourceKind) => {
    ...
    return (
        <>
            <SingleSelectAdapterResourceKind labels={labels.components.functions.AdapterKindSelect} fetchedOptions={adapterKinds} changeFunction={onAdapterKindChange} value={builderOptions.functions.adapterKind} />
            <SingleSelectAdapterResourceKind labels={labels.components.functions.ResourceKindSelect} fetchedOptions={resourceKinds} changeFunction={onResourceKindChange} value={builderOptions.functions.resourceKind} />
        </>
    );
};

export type MetricPropertyTag = {
    datasource: DataSource;
    builderOptions: QueryBuilderOptions;
...
export const MetricPropertyTag = (props: MetricPropertyTag) => {
    ...
    return (
       <>
           <SingleSelectAdapterResourceKind labels={labels.components.collectors.WithMetricSelect} fetchedOptions={metrics} changeFunction={onWithMetricChange} value={builderOptions.functions.withMetric} />
           <MultiSelectMetricPropertyTag labels={labels.components.collectors.WithPropertySelect} fetchedOptions={properties} changeFunction={onWithPropertyChange} values={builderOptions.collectors.withProperty || []}  />
       </>
    );
};
...
export const SelectForm = (props: SelectFormProps) => {
    const { datasource, filters, builderOptions, onWithMetricChange, onWithPropertyChange, onAdapterKindChange, onResourceKindChange} = props;
    return (
            <Stack direction='row' wrap='wrap' alignItems='start' justifyContent='start' gap={0}>

                <AdapterResourceKind datasource={datasource} builderOptions={builderOptions} onAdapterKindChange={onAdapterKindChange} onResourceKindChange={onResourceKindChange}/>
                <MetricPropertyTag datasource={datasource} builderOptions={builderOptions} onWithMetricChange={onWithMetricChange} onWithPropertyChange={onWithPropertyChange}/>
                        {
                            Object.keys(filters).map(name =>
                                <MultiSelect datasource={datasource} builderOptions={builderOptions}  values={builderOptions.filters[name as keyof Filters] || []} changeFunction={filters[name][1]} key={name} labels={filters[name][2]} useFetch={filters[name][3]}/>
                            )
                        }
            </Stack>
    );
